...
$CONTENT_TABLES_FULL = ['hm_data', 'services', 'inbox_messages', 'blog_posts'];
...
try {
  ...
  if ($action === 'insert' || $action === 'upsert') {
    ...
    // Plain insert/upsert. Booking slot reservation is owned by the timeline
    // endpoints (create-booking.php via hm_iv_reserve) — rest.php never band-reserves.
  ...
  if ($action === 'update') {
    ...
    // TIMELINE-ONLY: booking confirmation no longer runs a band/day/capacity check
    // here. A booking's reservation is its interval (start_at/end_at), locked at
    // create/reschedule via hm_iv_reserve; confirming is a plain status update.

    // Plain UPDATE. Booking rescheduling / slot moves are owned by reschedule.php
    // (hm_iv_reserve) — rest.php never band-reserves/releases.
  ...
  if ($action === 'delete') {
    ...
    // Plain DELETE (no band slot release — bookings carry no band reservations).
    $returned = [];
