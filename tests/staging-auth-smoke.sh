...
#
#  Runs the requirement-4 matrix that CANNOT be executed on a dev box without
#  PHP/MySQL: PHP lint → migration → seed → login/logout/password-change/role/
#  token/session tests. Run this ON the staging cPanel host (it has php + the
#  uploaded hm-api/ files + curl). The HTTP section can also be run from anywhere
#  that can reach the staging URL.
#
#  USAGE (on the staging host, from the site root):
...
#
#  Exit code 0 only if every check passes. Nothing here touches production.
# ════════════════════════════════════════════════════════════════════════════
set -u
...
if command -v php >/dev/null 2>&1; then
  for f in hm-api/_admin_users.php hm-api/admin-login.php hm-api/admin-migrate.php \
           hm-api/_lib.php; do
...
# ── 3. Login (valid) ──────────────────────────────────────────────────────────
hdr "3. login (valid)"
LOGIN="$(curl -s -X POST "$BASE/admin-login.php" $H_JSON $H_KEY -c /tmp/hm_cookies.txt \
...
BAD="$(curl -s -o /dev/null -w '%{http_code}' -X POST "$BASE/admin-login.php" $H_JSON $H_KEY \
...
V1="$(curl -s -X POST "$BASE/admin-login.php" $H_JSON $H_KEY -H "X-ADMIN-TOKEN: $TOKEN" -d '{"action":"verify"}')"
echo "$V1" | grep -q '"valid":true' && ok "good token → valid:true" || bad "good token not valid"
V2="$(curl -s -X POST "$BASE/admin-login.php" $H_JSON $H_KEY -H "X-ADMIN-TOKEN: forged.deadbeef" -d '{"action":"verify"}')"
echo "$V2" | grep -q '"valid":false' && ok "forged token → valid:false" || bad "forged token accepted"

# ── 6. list_users never exposes hashes (admin role) ───────────────────────────
hdr "6. list_users (admin) — no hashes"
LU="$(curl -s -X POST "$BASE/admin-login.php" $H_JSON $H_KEY -H "X-ADMIN-TOKEN: $TOKEN" -d '{"action":"list_users"}')"
...
# ── 7. Admin role: create a throwaway manager, then exercise role enforcement ──
hdr "7. admin role — create_user"
TMP_EMAIL="smoke+$RANDOM@hello-moving.com"; TMP_PASS="Smoke-$RANDOM-pw!"
CU="$(curl -s -X POST "$BASE/admin-login.php" $H_JSON $H_KEY -H "X-ADMIN-TOKEN: $TOKEN" \
...
# ── 8. Manager role: must NOT be able to manage accounts ──────────────────────
hdr "8. manager role enforcement"
if [ -n "$MGR_EMAIL" ] && [ -n "$MGR_PASS" ]; then
  MLOG="$(curl -s -X POST "$BASE/admin-login.php" $H_JSON $H_KEY \
  ...
  MCODE="$(curl -s -o /dev/null -w '%{http_code}' -X POST "$BASE/admin-login.php" $H_JSON $H_KEY \
...
# ── 9. Password change (own account) ──────────────────────────────────────────
hdr "9. change_password (own)"
NP="Rotated-$RANDOM-pw!"
CP="$(curl -s -X POST "$BASE/admin-login.php" $H_JSON $H_KEY -H "X-ADMIN-TOKEN: $TOKEN" \
  -d "{\"action\":\"change_password\",\"current\":\"$PASS\",\"new\":\"$NP\"}")"
echo "$CP" | grep -q '"ok":true' && ok "password changed" || { bad "change_password"; echo "    $CP"; }
# verify old password now rejected, new accepted, then restore original
OLD="$(curl -s -o /dev/null -w '%{http_code}' -X POST "$BASE/admin-login.php" $H_JSON $H_KEY -d "{\"action\":\"login\",\"email\":\"$EMAIL\",\"password\":\"$PASS\"}")"
[ "$OLD" = "401" ] && ok "old password rejected after change" || bad "old password still works (HTTP $OLD)"
NEWTOK="$(curl -s -X POST "$BASE/admin-login.php" $H_JSON $H_KEY -d "{\"action\":\"login\",\"email\":\"$EMAIL\",\"password\":\"$NP\"}" | jqget token)"
[ -n "$NEWTOK" ] && ok "new password works" || bad "new password rejected"
curl -s -X POST "$BASE/admin-login.php" $H_JSON $H_KEY -H "X-ADMIN-TOKEN: $NEWTOK" \
...
hdr "10. token revocation on delete (session/account expiry proxy)"
if [ -n "$NEWID" ]; then
  TLOG="$(curl -s -X POST "$BASE/admin-login.php" $H_JSON $H_KEY -d "{\"action\":\"login\",\"email\":\"$TMP_EMAIL\",\"password\":\"$TMP_PASS\"}")"
  TTOK="$(echo "$TLOG" | jqget token)"
  curl -s -X POST "$BASE/admin-login.php" $H_JSON $H_KEY -H "X-ADMIN-TOKEN: $TOKEN" \
    -d "{\"action\":\"delete_user\",\"id\":\"$NEWID\"}" >/dev/null
  REV="$(curl -s -o /dev/null -w '%{http_code}' -X POST "$BASE/admin-login.php" $H_JSON $H_KEY \
...
hdr "11. logout"
LO="$(curl -s -X POST "$BASE/admin-login.php" $H_JSON $H_KEY -b /tmp/hm_cookies.txt -c /tmp/hm_cookies.txt -d '{"action":"logout"}')"
...
echo "        To assert it deterministically, set admin_session_ttl=300 in the"
echo "        staging _config.php, wait >5 min, and re-run step 5 expecting valid:false."
