...
const CHANGELOG = [
  {
    version: 'v5.4', date: '2026-08-31', label: '最新',
    entries: [
      { type:'improve', text:'本番クローズ（最終安定化パス）：本番の稼働状態を検証（health ok/db true・トップ/予約/sw.js/env.js/health 正常・APIキー認証と管理エンドポイントの401ゲート維持・サービス画像6枚が正常表示＝有効なPNG・タイトル/canonical/robots/MovingCompany構造化データ/サイトマップ健全）。全テストスイート グリーン。セキュリティ点検（認証・認可・SQLプリペアドステートメント・アップロード検証・CORS・秘密情報の非コミット・エラー非漏洩）を実施。本番コード・DB・API・予約ロジックの変更は一切なし' },
      { type:'improve', text:'予約フォームの開始時刻スロットの信頼性回帰テストを堅牢化：読み込み状態の即時表示・旧スロットの即時クリア・順不同レスポンスの無視（レース保護）の検証を決定論化し、CIで常時実行するよう追加（先の「空きスロットちらつき」修正の回帰ガード）。本番の予約挙動・空き時間計算・APIは変更なし' },
      { type:'improve', text:'リポジトリ整理（フリーズ）：削除済み関数を参照する不要テストを撤去、main へ取り込み済み（squashマージ含む）のローカル／リモートブランチを全て整理、置き換え済みの旧PR6件をクローズ。main は origin と同期・作業ツリークリーン・本番／デプロイへの影響なし' },
    ]
  },
  {
    version: 'v5.3', date: '2026-08-30',
    entries: [
      { type:'improve', text:'スクロール追従バー（下部固定CTA）を整理：「お問い合わせ」ボタンを削除し、「今すぐ無料見積り」「マイ予約」の2つに絞って混雑を解消。ヘッダー・ヒーロー・#contactセクションのお問い合わせ、およびチャット機能・#contact導線は一切変更なし' },
      { type:'improve', text:'予約フォームの開始時刻スロット：選択中スロットの配色をHello Movingのお問い合わせグリーン（#06C755）に変更。テキストは可読性重視の濃色（コントラスト約8.3:1・WCAG AA準拠）、ホバー／フォーカスは濃いめの#05a847、空き＝中立色、無効＝淡色で区別。選択はラジオの:checkedで駆動し、予約・空き時間計算・API・バリデーションなどのロジックは変更なし。PC・モバイルとも視認性を確認' },
    ]
  },
  {
    version: 'v5.2', date: '2026-08-30',
    entries: [
      { type:'improve', text:'「引越しの流れ / How It Works」の本文をチャット中心の現在のご案内フローに刷新（電話中心の表現を撤廃）。5ステップ（お問い合わせ／チャットでご相談・無料お見積り／お引越し前のご準備／引越し当日／完了・最終確認）に更新し、CMS既定値（contentRegistry）も同一文言に統一。デザイン・カード・番号・レイアウト・アニメーション・data-content-key は変更なし' },
      { type:'fix', text:'「引越しの流れ」モバイル表示：番号サークル（01–05）とステップ本文の間隔を調整。サークルの実サイズ（58px）に合わせてグリッド列幅を修正し、サークルが本文に重なる問題を解消。縦位置の整列は維持・横スクロールなし（モバイルのみ・PCレイアウトは変更なし）' },
    ]
  },
  {
    version: 'v5.1', date: '2026-08-20',
    entries: [
      { type:'feat', text:'法的ページを新設：プライバシーポリシー（/privacy.html）と利用規約（/terms.html）を追加し、フッターの「プライバシーポリシー」「利用規約」リンクを実ページへ接続（従来は href="#" のプレースホルダー）。サイトのV2ダークテーマ（Noto Serif/Sans JP・ブランドヘッダー・無料見積りCTA・ライセンスフッター）に統一、レスポンシブ対応・noindex。施行日：2026年8月20日。自己完結ページのため Service Worker は変更なし（ナビゲーションは network-first で配信）' },
      { type:'improve', text:'⚠ 掲載中の法的文面は事業者ドラフト（暫定版）です。定義的な法的助言として利用する前に、事業者責任者または法務専門家によるレビュー・承認が必要。各ファイルのHTMLコメント・metaタグ（hm-legal-status）・ページ上の「暫定版」表示で明示。サイト既存の事実（社名・許可番号 第431320058126号・連絡先・サービス内容）のみ使用し、不明な項目（料金・キャンセル料・法人名・住所・管轄裁判所）は中立的表現とした（事実の創作なし）。予約フロー・API・DB・WAF・CMSは変更なし' },
    ]
  },
  {
    version: 'v5.0', date: '2026-08-20',
    entries: [
